...
async function councilSpeak(text,mode='pick'){
  ...
  try{
    ...
    const source=audioCtx.createBufferSource(),gain=audioCtx.createGain();gain.gain.value=.96;source.buffer=buffer;source.connect(gain);gain.connect(audioCtx.destination);councilVoiceSource=source;councilVoiceController=null;
    el?.classList.remove('voice-loading');el?.classList.add('voice-speaking');duckMusic(Math.max(3.5,buffer.duration+.8));source.onended=()=>{if(councilVoiceSource===source)councilVoiceSource=null;el?.classList.remove('voice-speaking');councilRestoreMusic()};source.start();
  }catch(e){if(e?.name==='AbortError')return;el?.classList.remove('voice-loading','voice-speaking');councilVoiceController=null;console.warn('Council voice unavailable',e);toast('AI voice uplink unavailable • text mode continues')}
}
...
showCouncilIntelligence=function(result,ctx,achievement,done){councilVoiceBasePick(result,ctx,achievement,()=>{councilStopVoice();done()});councilSpeak(result.text,'pick')};

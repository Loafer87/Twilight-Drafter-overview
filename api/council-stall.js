...
DELIBERATION: The table intentionally researches factions, so do not act as though thinking for a few minutes is inherently wrong. The joke is that YOUR patience is becoming unreasonable. Interruption 1 should feel like amused irritation or an unsolicited audit. Interruption 2 should escalate into an absurd administrative emergency, personal fascination, or offended disbelief. If previousInterruptions are supplied, do not repeat their core joke or wording.

TIME: elapsedSeconds is reliable and may be mentioned naturally. A temporal object may contain current local day/time and total session context; use it only if funny. If the player has visibly selected a faction but not locked it in, you may specifically react to that hesitation.
...
Write 2-4 punchy sentences, usually 45-90 words. Address or name the deliberating player. Make at least one observation specific to the offered factions or the current hesitation. On interruption 2, normally include one natural strong swear. Plain text only: no markdown, labels, bullets, headings, or quotation marks around the whole response.`;
...
module.exports=async function handler(req,res){
  ...
  if(req.method==='GET')return res.status(200).json({ok:true,configured:Boolean(key&&model),model:model||null,mode:'stall',knowledgeCore:true});

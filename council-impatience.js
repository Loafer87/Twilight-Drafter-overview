...
const COUNCIL_IMPATIENCE_WINDOWS=[{min:240,max:300},{min:420,max:510}];
...
function councilImpatienceEnsureUi(){
  if(!document.querySelector('#councilImpatienceStyle')){
    const style=document.createElement('style');style.id='councilImpatienceStyle';style.textContent=`
      ...
      .council-impatience-text{position:relative;font-size:18px;line-height:1.32;font-weight:600;color:#f3eefb;text-shadow:0 1px 10px rgba(0,0,0,.4)}
      ...
      @media(prefers-reduced-motion:reduce){.council-impatience{transition:none}.council-impatience-card:before{animation:none}}
...
function councilImpatienceHide(){const el=document.querySelector('#councilImpatience');el?.classList.remove('open','level-1','level-2')}
function councilImpatienceShow(text,player,elapsedSeconds,level){const el=councilImpatienceEnsureUi();el.classList.remove('level-1','level-2');el.classList.add(`level-${level}`);document.querySelector('#councilImpatienceLabel').textContent=level===1?'COUNCIL INTERRUPTION // DELIBERATION AUDIT':'COUNCIL INTERRUPTION // ADMINISTRATIVE EMERGENCY';document.querySelector('#councilImpatienceSubject').textContent=`${player} // ${councilImpatienceFormat(elapsedSeconds)}`;document.querySelector('#councilImpatienceText').textContent=text;requestAnimationFrame(()=>el.classList.add('open'));if(typeof councilIntelSound==='function')councilIntelSound();let hidden=false;const finish=()=>{if(hidden)return;hidden=true;councilImpatienceHide()};const fallback=setTimeout(finish,18000);if(typeof councilSpeak==='function'&&typeof councilVoiceEnabled!=='undefined'&&councilVoiceEnabled)councilSpeak(text,'pick',()=>{clearTimeout(fallback);setTimeout(finish,650)});else setTimeout(()=>{clearTimeout(fallback);finish()},9000)}

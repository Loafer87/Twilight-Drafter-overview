function councilOpeningContext(){
  const players=state.assignments.map((a,i)=>{
    ...
    return {name,order:i+1,speaker:i===0,history:{totalDraftPicks:history.total,factions:history.factions,speakerCount:history.speakerCount,achievements:history.achievements}};
  });
...
function councilLocalOpening(ctx){
  const returning=ctx.players.filter(p=>p.history.totalDraftPicks>0).sort((a,b)=>b.history.totalDraftPicks-a.history.totalDraftPicks);
